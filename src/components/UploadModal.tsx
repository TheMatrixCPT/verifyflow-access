...
interface UploadModalProps {
  open: boolean;
  onClose: () => void;
  onProcess: (sessionName: string, files: File[]) => void;
}

const UploadModal = ({ open, onClose, onProcess }: UploadModalProps) => {
  ...
  const handleProcess = () => {
    onProcess(sessionName || `Session ${new Date().toLocaleDateString()}`, files);
    setSessionName("");
    setFiles([]);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        >
          ...
          <p className="text-purple text-sm font-semibold mt-2">Process up to 100 documents at once</p>
          <input
            id="file-input"
            type="file"
            multiple
